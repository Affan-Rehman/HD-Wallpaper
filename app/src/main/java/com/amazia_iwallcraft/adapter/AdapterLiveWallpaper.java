...
public class AdapterLiveWallpaper extends RecyclerView.Adapter {
    ...
    public AdapterLiveWallpaper(Context context, ArrayList<ItemWallpaper> arrayList, RecyclerViewClickListener recyclerViewClickListener) {
    ...
    private void loadFav(final int posi, ConstraintLayout rootlayout) {
        if (sharedPref.isLogged()) {
            if (methods.isNetworkAvailable()) {
                Call<ItemSuccessList> call = APIClient.getClient().create(APIInterface.class).getDoFavourite(methods.getAPIRequest(Constant.URL_DO_FAV, 0, arrayList.get(posi).getId(), "", "", "", "", "", "", "", "", "", sharedPref.getUserId(), "LiveWallpaper"));
