...
public class FragmentWallFeatured extends Fragment {
    ...
    private void getWallpaperData() {
        if (methods.isNetworkAvailable()) {
            ...
            Call<ItemWallpaperList> call = APIClient.getClient().create(APIInterface.class).getWallpapersByFeatured(methods.getAPIRequest(Constant.URL_WALLPAPER_BY_FEATURED, 1, color_ids, wallType, "", "", "", "", "", "", "", "", new SharedPref(getActivity()).getUserId(), ""));
                    ...
                    if(getActivity() != null) {
                        if (response.body() != null && response.body().getArrayListWallpaper() != null) {
                            for (int i = 0; i < response.body().getArrayListWallpaper().size(); i++) {
                                dbHelper.addWallpaper(response.body().getArrayListWallpaper().get(i), "featured", "");
            ...
            arrayList = dbHelper.getWallByFeatured("1", wallType);
            setAdapter();
